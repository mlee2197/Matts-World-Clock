...
function updateWeather(){
	$.ajax(settings).done(function (response) {
		// weather data
		var weatherStart = response.indexOf("weather") + 10,
		    weatherEnd = response.indexOf("]"),
		    weather = response.substring(weatherStart, weatherEnd);  
		var weatherObject = JSON.parse(weather);

		//temperature data
		var tempStart = response.indexOf("temp") - 2,
			tempEnd = response.indexOf("visibility") - 2,
			temp = response.substring(tempStart, tempEnd);
		...
		// set weather text
		$("#weather").html(weatherObject.main);
		$("#temp").html(tempObject.temp);
		$("#temp-min").html(Math.floor(tempObject.temp_min));
		$("#temp-max").html(Math.ceil(tempObject.temp_max));
	});
}
...
var settings = {
	"async": true,
	"crossDomain": true,
	"url": "https://community-open-weather-map.p.rapidapi.com/weather?callback=test&id=2172797&units=imperial&mode=json&q=new%20york",
	"method": "GET",
	"headers": {
